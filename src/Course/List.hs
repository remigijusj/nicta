...
map f = foldRight (\h xs -> f h :. xs) Nil
...
filter p = foldRight (\h xs -> if p h then h :. xs else xs) Nil
...
seqOptional = foldRight consOptional (Full Nil)
  where consOptional = twiceOptional (:.)
...
-- (MY) find f = foldRight (\h o -> if f h then Full h else o) Empty
...
notReverse = reverse -- impossible, by induction
