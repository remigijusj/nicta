...
findLeft p (ListZipper l a r) = let (no, yes) = break p l in
  case yes of Nil      -> IsNotZ
              (h :. t) -> IsZ (ListZipper t h (reverse no ++ a :. r))
...
findRight p (ListZipper l a r) = let (no, yes) = break p r in
  case yes of Nil      -> IsNotZ
              (h :. t) -> IsZ (ListZipper (reverse no ++ a :. l) h t)
...
moveLeftLoop (ListZipper (h :. l) a r) = ListZipper l h (a :. r)
moveLeftLoop (ListZipper Nil a r) = let h :. t = reverse (a :. r) in ListZipper t h Nil
...
moveRightLoop (ListZipper l a (h :. r)) = ListZipper (a :. l) h r
moveRightLoop (ListZipper l a Nil) = let h :. t = reverse (a :. l) in ListZipper Nil h t
...
moveLeft (ListZipper (h :. l) a r) = IsZ (ListZipper l h (a :. r))
...
moveRight (ListZipper l a (h :. r)) = IsZ (ListZipper (a :. l)  h r)
...
swapLeft (ListZipper (h :. l) a r) = IsZ (ListZipper (a :. l) h r)
...
swapRight (ListZipper l a (h :. r)) = IsZ (ListZipper l h (a :. r))
...
moveRightN n z | n == 0 = IsZ z
               | n > 0  = moveRight -<< moveRightN (n-1) z
               | n < 0  = moveLeftN (-n) z
...
moveLeftN' n z@(ListZipper l _ r) =
  case moveLeftN n z of
    IsZ zz -> Right zz
    IsNotZ -> if n > 0 then Left (length l) else Left (length r)
...
moveRightN' n z@(ListZipper l _ r) =
  case moveRightN n z of
    IsZ zz -> Right zz
    IsNotZ -> if n > 0 then Left (length r) else Left (length l)
...
nth n z = let s = (n - index z) in moveRightN s z
...
deletePullLeft (ListZipper l a r) =
  case l of
    Nil    -> IsNotZ
...
deletePullRight (ListZipper l a r) =
  case r of
    Nil    -> IsNotZ
